...
/**
 * Encapsulates the conditional http request logic of <a href="https://tools.ietf.org/html/rfc7232">RFC
 * 7232</a>
 */
public class ETagEvaluator {
  ...
  public boolean shouldContentBeSend(@Nullable ETag currentETag,
      @Nullable ZonedDateTime currentLastModified) {
    if (currentETag != null) {
  ...
  public boolean isRequestLastModifiedOutdated(ZonedDateTime currentLastModified) {
    return getLastModified()
        .map(requestLastModified -> requestLastModified
            .isBefore(currentLastModified.truncatedTo(ChronoUnit.SECONDS)))
        .orElse(false);
  }
